        ...
        <div class="self-center rounded-xl mx-4 mb-4 grid bg-blue-400 flex justify-center items-center p-5 ">
            <div class="grid grid-cols-3 auto-cols-min gap-3 my-3">
                <?php foreach ($users as $key => $user): ?>
                    <div class="font-bold font-Dancing bg-slate-500 text-left text-white hover:bg-slate-600 flex items-center overflow-hidden px-4 py-2.5">
                        <?= "$user->id - $user->name"; ?>
                        <a href="/delete/<?= $user->id; ?>" style="color:red"  class="ml-2">del</a>
                        <a href="/edit/<?= $user->id; ?>" style="color:yellow"  class="ml-2">upd</a>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
